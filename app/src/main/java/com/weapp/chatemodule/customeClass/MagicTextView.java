...
public class MagicTextView extends androidx.appcompat.widget.AppCompatTextView {
	...
	private void init(AttributeSet attrs) {
		...
		if (attrs != null) {
			@SuppressLint("Recycle") TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.MagicTextView);

			if (a.hasValue(R.styleable.MagicTextView_foreground)) {
				Drawable foreground = a
						.getDrawable(R.styleable.MagicTextView_foreground);
				if (foreground != null) {
					this.setForegroundDrawable(foreground);
				} else {
					this.setTextColor(a.getColor(R.styleable.MagicTextView_foreground, 0xff000000));
					this.setStroke(1,R.color.black,strokeJoin,1);
				}
			}

			if (a.hasValue(R.styleable.MagicTextView_backgrounds)) {
				Drawable background = a
						.getDrawable(R.styleable.MagicTextView_backgrounds);
				if (background != null) {
					this.setBackgroundDrawable(background);
				} else {
					this.setBackgroundColor(a.getColor(
							R.styleable.MagicTextView_backgrounds, 0xff000000));
				}
			}

			if (a.hasValue(R.styleable.MagicTextView_innerShadowColor)) {
				this.addInnerShadow(a.getDimensionPixelSize(
						R.styleable.MagicTextView_innerShadowRadius, 0), a
						.getDimensionPixelOffset(
								R.styleable.MagicTextView_innerShadowDx, 0), a
						.getDimensionPixelOffset(
								R.styleable.MagicTextView_innerShadowDy, 0), a
						.getColor(R.styleable.MagicTextView_innerShadowColor,
								0xff000000));
			}

			if (a.hasValue(R.styleable.MagicTextView_outerShadowColor)) {
				this.addOuterShadow(a.getDimensionPixelSize(
						R.styleable.MagicTextView_outerShadowRadius, 0), a
						.getDimensionPixelOffset(
								R.styleable.MagicTextView_outerShadowDx, 0), a
						.getDimensionPixelOffset(
								R.styleable.MagicTextView_outerShadowDy, 0), a
						.getColor(R.styleable.MagicTextView_outerShadowColor,
								0xff000000));
			}
			...
			if (a.hasValue(R.styleable.MagicTextView_strokeColor)) {
				...
				int strokeColor = a.getColor(
						R.styleable.MagicTextView_strokeColor, 0xff000000);
		...
		if (innerShadows.size() > 0 || foregroundDrawable != null) {
			setLayerType(View.LAYER_TYPE_SOFTWARE, null);
		}
	}

	private void setStroke(float width, int color, Join join, float miter) {
		strokeWidth = width;
		strokeColor = color;
		strokeJoin = join;
		strokeMiter = miter;
	...
	private void addInnerShadow(float r, float dx, float dy, int color) {
		if (r == 0) {
			r = 0.0001f;
		}
	...
	private void setForegroundDrawable(Drawable d) {
	...
	private void generateTempCanvas() {
		...
		if (stored != null) {
			...
			canvasStore.put(key, new Pair<>(tempCanvas,
                    tempBitmap));
		}
	}
	...
	static class Shadow {
		...
		Shadow(float r, float dx, float dy, int color) {
			this.r = r;
			this.dx = dx;
			this.dy = dy;
			this.color = color;
